package demon.example.com.mylibrary.transition;
...
import demon.example.com.mylibrary.anim.SceneFade;
import demon.example.com.mylibrary.anim.SceneScaleUp;
import demon.example.com.mylibrary.anim.ViewAnim;
import demon.example.com.mylibrary.util.Position;
...
public class TransitionCompat {
	...
	private static void startSharedElementAnimation(final Activity activity, int layoutResId) {
		...
		/**
		 * 开始设定view开始时的大小和坐标位置(在sceneTranstionAnimation中已经设置过了)
		 */
		LayoutParams orginalParams = new LayoutParams(mWidth, mHeight);
		ViewGroup rootView = (ViewGroup)(activity.getWindow().getDecorView());
